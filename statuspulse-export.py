...
def main():
    if len(sys.argv) not in (2, 3, 4):
        raise SystemExit(
            "usage: statuspulse-export.py STATUSPAGE_URL [DAYS] [OUTPUT.csv]"
        )
    ...
    req = urllib.request.Request(
        ...
        headers={
            "User-Agent": "statuspulse-export/0.1 (+https://statusfeed-dev.github.io/statuspulse/)"
        },
    )
    ...
    for incident in payload.get("incidents", []):
        ...
        try:
            if (
                started
                and datetime.fromisoformat(started.replace("Z", "+00:00")) < cutoff
            ):
                continue
        except ValueError:
            pass
